...
const promised = (promise, options) => {
  ...
  return class extends React.PureComponent {
    ...
    componentDidUpdate(prevProps, prevState) {
      const { promiseCompleted, renderedWithValue } = this.state;

      // Wait until after rendering to set the `renderedWithValue` flag.
      // Allows preloadable components generated by the render function to
      // register to the preloader before announcing completion.
      if (!prevState.promiseCompleted && promiseCompleted)
        if (!renderedWithValue)
          this.setState({ renderedWithValue: true });
    }
    ...
    render() {
      const { renderedWithValue, promiseCompleted, promiseValue, error } = this.state;
      const preloaded = promiseCompleted && renderedWithValue;
